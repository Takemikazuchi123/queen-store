...
function validateForm() {
  ...
  if (!username){ alert(`🌸 Mohon masukkan ${['mlbb', 'pubg_id'].includes(metode) ? 'User ID' : 'Username'} kamu~`); usernameInput.focus(); return false; }
...
document.getElementById('orderBtn').addEventListener('click', () => {
  ...
  const unit           = metode === 'mlbb' ? 'Diamonds' : (metode === 'pubg_id' ? 'UC' : 'Robux');
  const displayNominal = isNaN(robux) ? robux : `${Number(robux).toLocaleString('id-ID')} ${unit}`;
  const idLabel        = ['mlbb', 'pubg_id'].includes(metode) ? 'User ID' : 'Username';
